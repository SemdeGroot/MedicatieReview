...
def genereer_word_document(patiënten_data, afdeling, output_path: str | None = None):
    ...
    for patiënt in patiënten_data:
        ...
        if patiënt.get("stopp"):
            ...
            for i, text in enumerate(headers):
                run = hdr_cells[i].paragraphs[0].add_run(text); run.bold = True
            ...
            for item in patiënt["stopp"]:
                ...
                row_cells[3].text = item.get('argument', '')
                row_cells[4].text = item.get('triggering_medicines', '')
        else:
        ...
        if patiënt.get("acb"):
            ...
            if middelen_met_bijdrage:
                ...
                for middel_info in middelen_met_bijdrage:
                    row_cells = table.add_row().cells
                    row_cells[0].text = middel_info.get('middel', '')
                    row_cells[1].text = str(middel_info.get('score', ''))
            else:
        ...
        for jansen_omschrijving in gesorteerde_keys:
            ...
            for label in ["Eerder besproken:", "Opmerking apotheker:", "Opmerking arts:\n"]:
                para = doc.add_paragraph(); run = para.add_run(label); run.bold = True
    ...
    if output_path:
        ...
        doc_path = os.path.join("Output", f"MedicatieReview_{afdeling}.docx")
        doc.save(doc_path)